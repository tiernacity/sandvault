...
install_lightpanda() {
    ...
    if [[ "$(uname -s)" == "Darwin" && "$(uname -m)" != "arm64" ]]; then
        abort "Lightpanda has no prebuilt macOS binary for $(uname -m). Use --chrome instead."
    fi
...
install_deps () {
    ...
    if [[ "$USE_BROWSER" == "true" ]]; then
        case "$BROWSER_KIND" in
            chrome)
                # Let the user choose between Google Chrome and Chromium
                ;;
            lightpanda)
                install_lightpanda
                ;;
...
start_lightpanda() {
    ...
    if [[ -z "$lightpanda_bin" ]]; then
        abort "lightpanda binary not found on PATH."
    fi
